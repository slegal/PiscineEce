...
function recupemploi()
{
    ...
    if($db_found) {
		...
		$sql = "SELECT* FROM emploi"; ///REQUETE SQL POUR AVOIR LA TABLE EMPLOI
		...
		foreach  ($db_handle->query($sql) as $row) {
            $tabemploi[$p][0] = $row['Num_utilisateur'];
			$tabemploi[$p][1] = $row['Description']; ///LIGNE A L INDICE $P , LA PREMIERE COLONNE = Desicription
			$tabemploi[$p][2] = $row['Date_creation']; ///LIGNE A L INDICE $P , LA DEUXIEME COLONNE = DATE CREATION
            $tabemploi[$p][3] = $row['Titre'];
            ...
            foreach  ($db_handle->query($sql2) as $row) {
                ...
                $tabemploi[$p][4] = $row['Nom']; ///LA 4eme CASE DU TABLEAU CONTIENT LE NOm
                $tabemploi[$p][5] = $row['Prenom']; ///LA 5 CASE DU TABLEAU CONTIENT LE PRENOM
